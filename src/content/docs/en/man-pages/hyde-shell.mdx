...
**`python-env`**
: Manage HyDE's isolated Python virtual environment. See [python-env](/en/man-pages/python-env/) for full documentation.

```bash
hyde-shell python-env create        # Create the virtual environment
hyde-shell python-env sync          # Sync dependencies from pyproject.toml
hyde-shell python-env install <pkg> # Install a package into the HyDE venv
hyde-shell python-env rebuild       # Destroy and recreate the venv
```
...
hyde-shell python-env          # Manages the HyDE Python venv
```
...
- **Python Environment Management**: Exposes `python-env` to create, sync, and manage the HyDE isolated venv
